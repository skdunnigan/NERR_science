...
EPR_NERR_simple <- function(Params){
  ...
  kmat <- kernmatsimple(Params$x,Params$F,Params,Params$T)# get fecuinsndity kernel
  ...
  #Integrate kernels using Simpsons
  #Sy <- makeSimpVec(Params$dx,length(Params.x))
  #Symat <-  repmat(Sy,length(Sy))
  # Integrate kernels
  #KR = Symat.*kmatNr;
  #KG = Symat.*kmatNg;
  #KM = Symat.*kmatNm;
  ...
  for (t in 2:TT){
    N[,t] = (kmat%*%N[,t-1])*Params$dx + cbind(Rdist)
  }
